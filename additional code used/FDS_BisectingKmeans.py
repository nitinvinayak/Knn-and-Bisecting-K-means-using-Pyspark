...
n=dataRDD.count()
FILE_NAME="outputBkm_k"+str(K)+"_n"+str(n)+".txt"
#os.system('cls')
ofile=open(FILE_NAME,"w")
for i in range(1,K+1):
    tempRDD=dataRDD.filter(lambda x: x[13]==i)
    ofile.write("CLUSTER NUMBER "+str(i)+" has "+str(tempRDD.count())+" elements"+"\n")
    ofile.write(str(tempRDD.collect())+"\n")



time_taken=(end - start)

ofile.write("It took "+str(time_taken)+" seconds")
